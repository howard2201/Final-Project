...
// Community presence from attendance records
$onlineResidents = [];
$offlineResidents = [];
try {
    $stmt = $pdo->prepare("CALL getAttendanceForAPI()");
    $stmt->execute();
    ...
    foreach ($attendanceRecords as $record) {
        if (empty($record['name']) || empty($record['time_in'])) continue;

        $name = $record['name'];
        $timeIn = strtotime($record['time_in']);
        $status = empty($record['time_out']) ? 'Online' : 'Offline';

        if (!isset($latestByName[$name]) || $timeIn > $latestByName[$name]['time_in']) {
            $latestByName[$name] = [
                'name' => $name,
                'status' => $status,
                'time_in' => $timeIn
    ...
    foreach ($latestByName as $info) {
        if ($info['status'] === 'Online') {
            $onlineResidents[] = $info['name'];
        } else {
            $offlineResidents[] = $info['name'];
        }
    }

    sort($onlineResidents);
    sort($offlineResidents);
} catch (PDOException $e) {
    error_log("Resident presence fetch error: " . $e->getMessage());
    $onlineResidents = [];
    $offlineResidents = [];
}
  ...
  <aside class="presence-panel">
    ...
    <div class="presence-group">
      <div class="presence-heading">
        ...
        <p>Online Now (<?php echo count($onlineResidents); ?>)</p>
      </div>
      <ul class="presence-list">
        <?php if (count($onlineResidents)): ?>
          <?php foreach ($onlineResidents as $name): ?>
            <li>
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <li class="muted">No one is online right now.</li>
        <?php endif; ?>
      </ul>
    </div>
    ...
    <div class="presence-group">
      <div class="presence-heading">
        ...
        <p>Offline (<?php echo count($offlineResidents); ?>)</p>
      </div>
      <ul class="presence-list">
        <?php if (count($offlineResidents)): ?>
          <?php foreach ($offlineResidents as $name): ?>
            <li>
              <span class="name"><?php echo htmlspecialchars($name); ?></span>
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <li class="muted">No offline records yet.</li>
        <?php endif; ?>
      </ul>
    </div>
  </aside>
